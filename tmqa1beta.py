...
# load settings
with open( "settings.json", "r") as settings_data:
...
#[ent.kb_id_ for ent in get_kb_ents(ent_text_test)]
...
#len(get_all_statements_of_entity("Q16614390"))
...
#get_wd_ids("wife", is_predicate=True , top_k=0, limit=0)
...
#cluster_extractor_from_complements(q0_themes[1])
...
#build_graph(q0_nlp_test, q0_themes_test, q0_themes_enhanced_test, q0_predicates_test, deep_k=3)
...
#is_id_in_graph(graph, "Q24039104")
...
def answer_initial_question(question):
    ...
    is_question = True if [t for t in q_nlp if t.tag_ in WH_FILTER] else False
    if not is_question:
        return False
...
#graph, anwser = answer_initial_question("Which actor voiced the Unicorn in The Last Unicorn?")
#graph, anwser = answer_initial_question("Who is the author of Le Petit Prince?")
#graph, anwser = answer_initial_question("Who made the soundtrack of the The Last Unicorn movie?")
#graph, anwser = answer_initial_question("When was produced the first Matrix movie?")
#graph, anwser = answer_initial_question("Who is the president of the United States?")
#graph, anwser = answer_initial_question("Who is the wife of Barack Obama?")
#anwser = answer_initial_question("what was the cause of death of yves klein")
#print(anwser)
...
def get_focused_parts_2(nlp_sentence, top_k=3):
    focused_parts = [t.head for t in nlp_sentence if t.tag_ == "WDT" or t.tag_ == "WP" or t.tag_ == "WP$" or t.tag_ == "WRB"] 
    focused_parts_ids = [get_wd_ids(p.text, top_k=top_k) for p in focused_parts]
    return merge_lists(focused_parts, focused_parts_ids)

#get_focused_parts_2(q0_nlp_test)
...
#q_focused_parts_2 = get_focused_parts_2(q_nlp_2)
#print("q_nlp:", q_nlp_2)
...
#path_nodes_2 = find_path_nodes_from_graph_2(graph_2, paths_keywords_2, threshold=0.8)
#path_nodes_2
...
def get_hypothesises(nlp, paths_keywords, filtered_paths):
    ...
    #print("anchors_focuses",anchors_focuses)
    ...
    anchors_predicates = sorted(anchors_predicates, key=lambda x: x[-1], reverse=True)
...
#hypothesises_2 = get_hypothesises(q_nlp_2, paths_keywords_2, paths_nodes_filtered_2)
...
def answer_question(question):
    ...
    #print("q_predicates",q_predicates)
    q_focused_parts = get_focused_parts_2(q_nlp)
    #print("q_focused_parts",q_focused_parts)
    graph, predicates_dict = build_graph(q_nlp, q_themes, q_themes_enhanced, q_predicates)
    ...
    path_nodes = find_path_nodes_from_graph_2(graph, paths_keywords, threshold=0.7, top_k=5)
...
#print(answer)
#if answer: print([get_wd_label(h[0]) for i,h in enumerate(answer) if i < 3])
...
#is_id_in_graph(graph_2, "P26-1")
#
#
## In[ ]:
#
#
#def enhance_paths_with_question_anchor(graph, path_nodes, anchor):
#    
#    return [p for p in path_nodes]
#
#enhance_paths_with_question_anchor(graph_2, path_nodes_2, anchor)
#
#
## In[ ]:
#
...
#
#
#nlp_test_5 = get_nlp("yves klein")
#nlp_test_6 = get_nlp("klein yves")
#nlp_test_7 = get_nlp("Yves Klein")
#print(nlp_test_6.similarity(nlp_test_7))
#print(nlp_test_5.similarity(nlp_test_6))
#
#
## In[ ]:
#
