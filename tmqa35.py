...
def get_all_simple_paths_worker(graph, in_mp_queue, out_mp_queue, max_paths_proc_killer):
    found_paths = []
    sentinel = None
    killer_trigger = False
    for source, target in iter(in_mp_queue.get, sentinel):
        for path in nx.all_simple_paths(graph, source = source, target = target, cutoff = None):
            ...
            if len(found_paths) >= max_paths_proc_killer:
                killer_trigger=True
                break
        if killer_trigger:
            break
...
def get_paths_keywords_nodes(graph, keywords,threshold=0.9,top_performance=50, per_limit=70, max_paths=300000, max_paths_proc_killer=False ,cores=mp.cpu_count()):
    if not max_paths_proc_killer:
        max_paths_proc_killer = max_paths/2
    ...
    #print("len(keywords_nodes)",len(keywords_nodes))
    ...
    if keywords_nodes:
        ...
        if len(keywords_nodes) > 1:
            ...
            #print("len(keywords_nodes_per)",len(keywords_nodes_per))
            ...
            #print("IF len(keywords_nodes_per)",len(keywords_nodes_per))
            
            if len(keywords_nodes_per) > per_limit:
                keywords_nodes_per = [(keywords_nodes+keywords_nodes)]
                #print("LIMIT TRIGGERED: len(keywords_nodes_per)",len(keywords_nodes_per))
                            
        else:
            keywords_nodes_per = [(keywords_nodes+keywords_nodes)]
            #print("<1 len(keywords_nodes_per)",len(keywords_nodes_per),keywords_nodes_per[0])
            #print("ELSE len(keywords_nodes_per)",len(keywords_nodes_per))
    ...
    product_list = list(it.product(sources, targets))
    #print("len(product_list)",len(product_list))
    for source, target in it.product(sources, targets):
        in_mp_queue.put((source, target))

    procs = [mp.Process(target = get_all_simple_paths_worker, args = (graph, in_mp_queue, out_mp_queue, max_paths_proc_killer)) for i in range(cores)]
    
    killing_toggle = False
    ...
    for proc in procs:
        ...
        #print("Time spent so far:", str(time.time()-start_time_proc))
        #print("PROC len(paths_keyword_nodes)",len(paths_keyword_nodes))
        if len(paths_keyword_nodes) >= max_paths:
            #print("BREAK len(paths_keyword_nodes) >= max_paths")
            killing_toggle = True
            break
    for proc in procs:
        #print("IN THE JOINER")
    ...
    #print("before len(paths_keyword_nodes)",len(paths_keyword_nodes))
    
    paths_keyword_nodes = [p for p in paths_keyword_nodes if p]
    #print("after len(paths_keyword_nodes)",len(paths_keyword_nodes))
...
def find_path_nodes_from_graph(nlp_question, graph, predicates_dict, keywords, threshold=0.9,special_pred_theshold=0.7, thres_inter=0.15, top_performance=50,min_paths=3000, max_paths=300000, sim_paths_threshold=0.5, sim_looped=False, cores=mp.cpu_count()):
    ...
    if not date_trigger: date_trigger = is_time_question(nlp_question)
    ...
    all_keywords = keywords[0] + option_keywords + k1_predicates
    
    all_keywords_filtered = []
    [all_keywords_filtered.append(k) for k in list(all_keywords for all_keywords,_ in it.groupby(all_keywords)) if k]
    all_keywords = all_keywords_filtered
    
    
    
    #print("len(all_keywords)",len(all_keywords))
    #print("all_keywords",all_keywords)
    
    new_all_keywords = []
    for k in all_keywords:
        k_sim = get_nlp(k).similarity(nlp_question)
        if k_sim > sim_paths_threshold:
            new_all_keywords.append(k)
            
    all_keywords = new_all_keywords
    #print("filtered len(all_keywords)",len(all_keywords))
    #print("filtered all_keywords",all_keywords)
    
    main_keyword_paths = get_paths_keywords_nodes(graph, all_keywords,threshold=threshold,top_performance=top_performance,max_paths=max_paths, cores=cores)
    ...
    #keyword_paths = list(set(keyword_paths))
    #print("before keyword_paths[:10]",keyword_paths[:10])
    keyword_paths = list(keyword_paths for keyword_paths,_ in it.groupby(keyword_paths))
    #print("after keyword_paths[:10]",keyword_paths[:10])
    #print("END len(keyword_paths)",len(keyword_paths))
    
    if len(keyword_paths) == 0 and sim_paths_threshold <= 1.0:
        sim_looped = True
        sim_paths_threshold += thres_inter
        keyword_paths = find_path_nodes_from_graph(
                                    nlp_question, graph, predicates_dict, keywords,
                                    threshold=threshold,special_pred_theshold=special_pred_theshold, thres_inter=thres_inter,
                                    top_performance=top_performance, min_paths=min_paths, max_paths=max_paths,
                                    sim_paths_threshold=sim_paths_threshold,
                                    sim_looped=sim_looped, cores=cores
                                                  )
        #print("ELIF LOOPING")
        #print("len(keyword_paths) == 0 and sim_paths_threshold <= 1.0 and sim_looped")
        #print("len(keyword_paths)",len(keyword_paths))
        #print("threshold",threshold)
        #print("sim_paths_threshold",sim_paths_threshold)
    
    elif len(keyword_paths) <= min_paths and len(keyword_paths) <= max_paths + max_paths/2:
        if threshold == 0: return keyword_paths
        threshold -= thres_inter
        if threshold < 0: threshold = 0
        #print("IF LOOPING")
        #print("len(keyword_paths) <= min_paths and len(keyword_paths) <= max_paths + 10000")
        #print("len(keyword_paths)",len(keyword_paths))
        #print("threshold",threshold)
    ...
    elif len(keyword_paths) > max_paths and threshold < 1.0:
        if threshold == 0.99: return keyword_paths
        threshold += thres_inter
        if threshold >= 1.0: threshold = 0.99
        #print("ELIF LOOPING")
        #print("len(keyword_paths) > max_paths and threshold < 1.0")
        #print("len(keyword_paths)",len(keyword_paths))
        #print("threshold",threshold)
        #keyword_paths = get_paths_keywords_nodes(graph, all_keywords,threshold=threshold,top_performance=top_performance, cores=cores)
        keyword_paths = find_path_nodes_from_graph(
    ...
    elif len(keyword_paths) > max_paths and sim_paths_threshold <= 1.0 and not sim_looped:
        #print("ELIF LOOPING")
        #print("len(keyword_paths) > max_paths and sim_paths_threshold <= 1.0 and not sim_looped")
        #print("len(keyword_paths)",len(keyword_paths))
        #print("threshold",threshold)
        #print("sim_paths_threshold",sim_paths_threshold)
        sim_paths_threshold += thres_inter
        ...
        #keyword_paths = get_paths_keywords_nodes(graph, new_all_keywords,threshold=threshold,top_performance=top_performance, cores=cores)
...
def is_time_question(nlp_question):
    DATE_KEYWORDS = ["date", "day", "month"]
    W_TRIGGERS = ["WP","WDT"]
    
    #print([(t, t.lemma_, t.lower_, t.tag_, t.pos_, [c for c in t.children]) for t in nlp_question])
    
    if 'when' in [t.lower_ for t in nlp_question if t.tag_=="WRB"]:
        #print("1 here")
        return True
    else:
        time_triggers = [t.lower_ for t in nlp_question if t.tag_ in W_TRIGGERS]
        if 'what' in time_triggers or "which" in time_triggers:
            #print("2 here")
            triggers_children = [[(c.lower_,t.lower_) for c in t.children] for t in nlp_question]
            #print("triggers_children",triggers_children)
            for tc in triggers_children:
                for e in tc:
                    if (e[0] == 'what' or e[0] == 'which') and e[1] in DATE_KEYWORDS:
                        #print("3 here")
                        return True
            triggers_children = [[c.lower_ for c in t.children] for t in nlp_question]
            for tc in triggers_children:
                #print("4 here")
                if "what" in tc and any([any(a) for a in [[e==etc for e in DATE_KEYWORDS] for etc in tc]]):
                    #print(any([any(a) for a in [[e==etc for e in DATE_KEYWORDS] for etc in tc]]))
                    return True
            #if 
            return False
        return False
    
#is_time_question(get_nlp("What was the love of publication for the first book"))
...
def get_hypothesises(nlp, predicates_dict, predicates, themes, paths_keywords, filtered_paths, threshold=0.5, special_pred_theshold=0.7, max_reward=2.0, in_context=False):#, themes, themes_enhanced):
    ...
    date_trigger = "date" in w_names_only
    if not date_trigger: date_trigger = is_time_question(nlp)
...
def convert_to_literal(to_literal, question):
    #print("to_literal",to_literal)
    DATE_WORDS = ["year", "month", "day", "date"]
    if is_timestamp(to_literal):
        datetimeObj = datetime.strptime(to_literal, '%Y-%m-%dT%H:%M:%SZ')
        question_list = question.lower().split(" ")
        if (any([any(e) for e in [[item==item2 for item in DATE_WORDS] for item2 in question_list]]) and
            ("what" in question_list or "which" in question_list)):
            #print("1 here")
            return str(datetimeObj.year)
        elif str(datetimeObj.time())=="00:00:00" and str(datetimeObj.date())[5:]=="01-01":
            #print("2 here")
            return str(datetimeObj.year)
        elif str(datetimeObj.time())=="00:00:00":
            return str(datetimeObj.strftime("%d %B %Y"))
        else:
            return str(datetimeObj.strftime("%d %B %Y")+" at "+datetimeObj.strftime("%H:%M"))
    else:
        return to_literal
    
#convert_to_literal("1997-01-01T00:00:00Z","What was the date of publication for the first book?")
...
def answer_question(question, verbose=False, aggressive=False, looped=False,
                    ...
                    paths_filter_max=10000, paths_max=8000,
    ...
    time_sensitive = is_time_question(q_nlp)
    binary_question = is_binary_question(q_nlp)
    if verbose and time_sensitive: print("> Time related question detected")
    if verbose and binary_question: print("> Binary question related question detected")
    ...
    path_nodes = find_path_nodes_from_graph(q_nlp, graph, predicates_dict, paths_keywords, threshold=0.8,special_pred_theshold=0.7, thres_inter=0.1, top_performance=len(graph), min_paths=100, max_paths=paths_max, sim_paths_threshold=0.5, cores=cores)
...
#if answer_1:
#    print("Answer:",convert_to_literal(get_wd_label(answer_1[0][0]),question), "("+str(answer_1[0][0])+")\n")
...
conversation_questions = [
    ...
    "What was the year of publication for the first book?",
    "The first book was called what?",
...
def answer_conversation(questions, answer_context=False, context_graph=False, start_from=0, end_at=6):
    ...
    for i_q,question in enumerate(questions):
        ...
        if answer_context: print("Answer:",convert_to_literal(get_wd_label(answer_context[0][0]),question), "("+str(answer_context[0][0])+")\n")
        else: print("Answer:",answer_context)
        #break
    return conversation_history

#conversation_history_0 = answer_conversation(conversation_questions,answer_context=False,context_graph=False, start_from=0, end_at=0)

#answer_context_0 = conversation_history_0[0][0]
#context_graph_0 = conversation_history_0[0][1]
#conversation_history_1 = answer_conversation(conversation_questions,answer_context=answer_context_0,context_graph=context_graph_0, start_from=1, end_at=1)

#answer_context_1 = conversation_history_1[1][0]
#context_graph_1 = conversation_history_1[1][1]
#conversation_history_2 = answer_conversation(conversation_questions,answer_context=answer_context_1,context_graph=context_graph_1, start_from=2, end_at=2)

#answer_context_2 = conversation_history_2[2][0]
#context_graph_2 = conversation_history_2[2][1]
#conversation_history_3 = answer_conversation(conversation_questions,answer_context=answer_context_2,context_graph=context_graph_2, start_from=3, end_at=6)
#
#answer_context_3 = conversation_history_3[2][0]
#context_graph_3 = conversation_history_3[2][1]
#conversation_history_4 = answer_conversation(conversation_questions,answer_context=answer_context_3,context_graph=context_graph_3, start_from=3, end_at=6)
...
#k1_predicates_test = ['author', 'instance of', 'day in year for periodic occurrence', 'date of publication', 'series', 'present in work']
#option_keywords_test = ['date of first performance', 'date of publication', 'place of publication', 'date of publication', 'end date', 'start date', 'date of birth', 'date of death', 'author', 'characters', 'character role']
#keywords_0_test = ['first', 'publication', 'date', 'book', 'harry potter', 'j. k. rowling', 'film character', 'first book']
#question_test = "What was the year of publication for the first book?"
#all_keywords_test = k1_predicates_test+option_keywords_test+keywords_0_test
#
##all_keywords_test = ['series', 'harry potter', 'author', 'the harry potter series', 'the author', 'henry potter', 'instance of', 'series ordinal']
##question_test = "Who is the author of the Harry Potter series?"
#for k in all_keywords_test:
#    k_sim = get_nlp(k).similarity(get_nlp(question_test))
#    if k_sim > 0.5:
#        print(k, k_sim)
...
def generate_answer_sentence(answer, question, model, tokenizer, verbose=False, k_spo=False, g_autocorrect=True):
    if answer:
        ...
        for i_e,e in enumerate(answer_best_path_spos):
            e = convert_to_literal(e, question)
            if i_e == 0:
                raw_list.append('[CLS]')
...
#        if answer:
#            print("Answer:",convert_to_literal(get_wd_label(answer[0][0]), question), "("+str(answer[0][0])+")\n")
...
def get_top_1_answer(answer_convex,question):
    time_sensitive = is_time_question(get_nlp(question))
    ...
    for i_q,question in enumerate(questions):
        ...
        if i_q >= 1:
            if i_q == 0:
                ...
                if answer_convex: print("Answer:",convert_to_literal(get_wd_label(answer_convex[0][0]), question), "("+str(answer_convex[0][0])+")\n")
            elif context_graph:
                print("Context Question:",question)
                ...
                plot_graph(context_graph, "file_name_context_graph", "Context_Graph_title")
                if top_1_answer: print("Answer:",convert_to_literal(get_wd_label(top_1_answer), question), "("+str(top_1_answer)+")\n")
...
#plot_graph(context_graph, "file_name_context_graph", "Context_Graph_title")
#if top_1_answer: print("Answer:",convert_to_literal(get_wd_label(answer_convex[0]["answer"]),question), "("+str(answer_convex[0]["answer"])+")\n")
#for ac in answer_convex:
#    print(ac)
#print("\n")
#
