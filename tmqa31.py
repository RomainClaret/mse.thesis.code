...
def get_themes_ids_from_names(noun_chunks, top_k=3, online=False):
    if online:
        theme_ids = [get_wd_ids_online(chunk, top_k=top_k)+get_wd_ids(chunk, top_k=top_k) for chunk in noun_chunks]
    else:
        theme_ids = [get_wd_ids(chunk, top_k=top_k) for chunk in noun_chunks]
    return theme_ids
...
# question: nlp_string
#limits=plt.axis('off')
def build_graph(nlp, themes, themes_enhanced, predicates, deep_k=3, time_sensitive = False, cores=mp.cpu_count(), context_graph=False, previous_answer=False, aggressive=False, k_deep_followup=2):
    ...
    if context_graph:# and not aggressive:
        ...
        predicates_ids = previous_answer_predicates_ids+predicates_ids[:k_deep_followup]
        #print("build_graph - not agressive and context graph present")
        
    elif context_graph:
        #print("build_graph - agressive and context graph present")
        theme_ids += previous_answer_entity_ids[:k_deep_followup*2]
        predicates_ids = previous_answer_predicates_ids[:k_deep_followup*2]
...
def answer_question(question, verbose=False, aggressive=False, looped=False,
                    ...
                    deep_match=4, k_context=1, in_context=False, k_deep_followup=2,
        ...
        if previous_answer:
            ...
            if len(previous_answer) > 1:
                ...
                for lg in last_golden_answer_entities:
                    ...
                    lg_ids = get_wd_ids(lg_name, is_predicate=False, top_k=deep_k, limit=deep_k+1, online=False)
    ...
    q_theme_enhanced_names = [q[0] for q in q_themes_enhanced]
    meaningful_names = q_theme_names+q_theme_enhanced_names
    ...
    if context_graph:
        ...
        [meaningful_names.append(w) for w in context_centers_names if w not in meaningful_names] 
        ...
        for mnnpai in get_themes_ids_from_chunks(meaningful_names_no_previous_answer):
            tmp_row = []
            for mn in mnnpai:
                if mn in last_golden_answer_entities+q_theme_ids:
                    tmp_row.append(mn)
    ...
    if deep_k<2:
        deep_k = 2
        graph, predicates_dict = build_graph(q_nlp, q_themes, q_themes_enhanced, q_predicates, deep_k=deep_k, time_sensitive=time_sensitive, cores=cores, context_graph=context_graph,previous_answer=previous_answer,aggressive=aggressive,k_deep_followup=k_deep_followup)
    ...
    if g_paths:
        if hypothesises:
            ...
            golden_paths = match_hypothesises(graph, q_nlp, q_themes, q_predicates, hypothesises, paths_nodes_filtered, threshold=0.8, max_reward=2.0,winner_threshold_diff=4.0, time_sensitive=time_sensitive,deep_match=deep_match)
            ...
            if not looped and not aggressive:
                ...
                golden_paths = answer_question(question, verbose=verbose, aggressive=True, looped=True,
                    ...
                    deep_match=deep_match, k_context=k_context, in_context=in_context, k_deep_followup=2,
                    k_deep_context_graph=k_deep_context_graph
                    )
            else: 
    ...
    if g_paths:
        if golden_paths:
            ...
            if show_graph:
                #print("PLOT 2")
                if verbose: print("---> Ploting the previous context graph")
                plot_graph(previous_graph, "file_name_context_graph", "Context_Graph_title")
...
#conversation_history = answer_conversation(conversation_questions,answer_context=first_answer,context_graph=first_context, start_from=1, end_at=6)
