...
#get_entity_similarity("Q131281", "location", [], max_reward=2)
                        ...
                        if ap[0] == ht[1]:
                            ...
                            for wn in w_names_only:
                                for es in get_entity_similarity(ht[0], wn, anchors_focuses, max_reward=max_reward):
                                    ht0_sum += es
                                    if tmp_to_find in ht: print("w_names_only ht0_sum ht[0], wn, es",ht[0], wn, es, ht0_sum)
                                for es in get_entity_similarity(ht[2], wn, anchors_focuses, max_reward=max_reward):
                                    ht2_sum += es
                                    if tmp_to_find in ht: print("w_names_only ht2_sum ht[2], wn, es",ht[2], wn, es, ht2_sum)
                                ...
                                if ht2_label == tk.text.lower() and ht[1] in main_predicate_ids:
                                    for wn in w_names_only:
                                        for es in get_entity_similarity(ht[0], wn, anchors_focuses, max_reward=max_reward*2):
                                            ...
                                            ht2_sum += es
                                            if tmp_to_find in ht: print("theme_keywords ht2_sum ht[0], wn, es",ht[0], wn, es, ht2_sum)
...
def match_hypothesises(graph, question, themes, predicates, hypothesises, paths, threshold=0.8, max_reward=2.0, winner_threshold_diff=0.3, time_sensitive=False, cores=mp.cpu_count()):
    BANNED_IDS = ["P31"]
    LOCATION_FILTER = ["GPE", "FAC", "LOC","PERSON"]
...
def answer_question(question, verbose=False, aggressive=False, looped=False, deep_k=2, deep_k_step=1, deep_k_max=20,graph_size_min=100, graph_size_max=350, timer=False, g_paths=True, show_graph=False, cores=mp.cpu_count()):
    if verbose: start_time = time.time()
    if timer: timer_time = time.time()
    if verbose: print("Auto correcting question:",question)
    q_nlp = get_nlp(question, autocorrect=True)
    ...
    if verbose: print("-> Computing possible paths... (could be long)")
    path_nodes = find_path_nodes_from_graph(graph, paths_keywords, threshold=0.8, thres_inter=0.1, top_performance=len(graph), min_paths=100, cores=cores)
    if verbose: print("--> len(path_nodes):",len(path_nodes))
    if timer: 
        print("->\tRunning time is {}s".format(round(time.time()-timer_time,2)))
        timer_time = time.time()
    ...
    if g_paths:
        if hypothesises:
            ...
            golden_paths = match_hypothesises(graph, q_nlp, q_themes, q_predicates, hypothesises, paths_nodes_filtered, threshold=0.8, max_reward=2.0, time_sensitive=time_sensitive)
