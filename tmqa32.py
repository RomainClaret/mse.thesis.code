...
#conversation_history_2 = answer_conversation(conversation_questions,answer_context=first_answer,context_graph=first_context, start_from=1, end_at=6)
...
def extend_answer_sentence(sentence, verbose=False, k_sentences=False):
    BREAKING_TOKENS = [".","!","?"]
    
    tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
    model = GPT2LMHeadModel.from_pretrained("gpt2")
    
    sentence += generate_next_word(sentence, model, tokenizer, threshold=0.7)
    done = False
    if k_sentences:
        #k_sentences -= 1
        while not done:
            if verbose: print("\r\t>>> Building sentence: {}".format(sentence), end='                                  ')
            next_word = generate_next_word(sentence, model, tokenizer, threshold=0.7)
            sentence += next_word
...
#sentence = "Yes, and"
#extend_answer_sentence(sentence, verbose=True, k_sentences=2)
